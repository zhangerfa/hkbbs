...
export class User {
    ...
    static login(args: { rememberMe?: boolean, stuId: string, password: string }): Promise<ResultBoolean> {
        return req.post({
            url: this.path + "login",
            data: args
        })
    ...
    static register(args: { code: string, stuId: string, username: string, password: string, gender: "0" | "1" }): Promise<ResultBoolean> {
        return req.post({
